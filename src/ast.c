...
const ASTNode *new_TupleNode(struct YYLTYPE *loc, const Vector *exprs) {
    ...
    vtable->get_vars = GetVars_Tuple;
    vtable->get_name = GetName_Tuple;
...
static void free_function(const void *this) {
    ...
    if (data->self) data->self->free(data->self, NULL);
    free(data->loc);
    free(node->data);
    free(node->vtable);
...
const ASTNode *new_TypedVarNode(
        struct YYLTYPE *loc, char *name, VarType *type
) {
    ASTNode *node = malloc(sizeof(*node));
    if (node == NULL) {
        return NULL;
