...
static void define_builtins(FILE *out) {
    ...
    fprintf(out, "}\n");
    fprintf(out, "\n");
    fprintf(out, "void *builtin_0x%X(closure *c, void *a, void *b) {\n", '+');
    print_indent(1, out);
    fprintf(out, "return new_Int(((class_Int*)a)->value + "
                 "((class_Int*)b)->value);\n");
    fprintf(out, "}\n");
    fprintf(out, "\n");
}
...
static int define_classes(CodegenState *state, FILE *out) {
    ...
    for (size_t i = 0; i < BUILTIN_COUNT; i++) {
        ...
        size_t      field_count;
        safe_method_call(type->class->fields, array, &field_count, &fields);
        ...
        for (size_t i = 0; i < fn_count; i++) {
            ...
            int         no_vars    = 1;
            {
                ...
                for (size_t j         = 0; j < symbol_count; j++) {
                    ...
                    if (data->self
                            ->contains(data->self, symbols[j], lengths[j])) {
                        free((char *) symbols[j]);
                        continue;
                    }
                    VarType *type = NULL;
...
char *CodeGen_Return(const void *this, void *state, FILE *out) {
    ...
    if (data->returns != NULL) {
        ...
        char               *code       =
                                   ret_vtable->codegen(ret_node, state, out);
...
char *CodeGen_Ref(const void *this, void *state, FILE *out) {
    ...
    char               *code       =
                               ref_vtable->codegen(data->expr, state, out);
...
char *CodeGen_Function(const void *this, void *state, FILE *out) {
    ...
    for (size_t j         = 0; j < symbol_count; j++) {
        ...
        fprintf(out, "%s(void*)%.*s", sep, (int) lengths[j], symbols[j]);
        sep = ", ";
        free((void *) symbols[j]);
    }
    free(lengths);
